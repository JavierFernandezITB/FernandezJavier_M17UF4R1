...
public class AIIdleState : AIBaseState
{
    ...
    public override void EnterState(AIHandler handler)
    {
        ...
        aiAgent = handler.GetComponent<NavMeshAgent>();
        aiAgent.stoppingDistance = 0;
        aiAgent.ResetPath();
        if (handler.currentNodeRegion && !handler.currentNodeRegion.agentsInRegion.Contains(handler))
        {
    ...
    public override void UpdateState(AIHandler handler)
    {
        ...
        else if (handler.currentNodeRegion != null && !switchNodeDebounce)
        {
            ...
            {
                if (aiAgent.remainingDistance == 0 && !aiAgent.pathPending)
                {
                    aiAgent.destination = nextNode.transform.position;
                    nextNode.isOccupied = true;
                    handler.currentNode = nextNode;
                    nodeCoroutine = handler.StartCoroutine(DelayNextNode(aiAgent, handler));
                }
            }
        }
    }
    ...
    public override void ExitState(AIHandler handler)
    {
        ...
        try
        {
            handler.StopCoroutine(nodeCoroutine);
        }
        catch { }
    }
